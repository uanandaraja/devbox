...
complete -c bun -n $flag_applies --no-files -s 'u' -l 'origin' -r -d 'Server URL. Rewrites import paths'
complete -c bun -n $flag_applies --no-files -s 'p' -l 'port' -r -d 'Port number to start server from'
complete -c bun -n $flag_applies --no-files -s 'd' -l 'define' -r -d 'Substitute K:V while parsing, e.g. --define process.env.NODE_ENV:\"development\"'
complete -c bun -n $flag_applies --no-files -s 'e' -l 'external' -r -d 'Exclude module from transpilation (can use * wildcards). ex: -e react'
complete -c bun -n $flag_applies --no-files -l 'use' -r -d 'Use a framework (ex: next)'
complete -c bun -n $flag_applies --no-files -l 'hot' -r -d 'Enable hot reloading in Bun\'s JavaScript runtime'
...
complete -c bun -n "__fish_seen_subcommand_from bun" -F -d 'Bundle this'
complete -c bun -n "__fish_seen_subcommand_from create; and __fish_seen_subcommand_from react next" -F -d "Create in directory"
complete -c bun -n "__fish_use_subcommand" -a 'init' -F -d 'Start an empty Bun project'
complete -c bun -n "__fish_use_subcommand" -a 'install' -f -d 'Install packages from package.json'
complete -c bun -n "__fish_use_subcommand" -a 'add' -F -d 'Add a package to package.json'
complete -c bun -n "__fish_use_subcommand" -a 'remove' -F -d 'Remove a package from package.json'
...
complete -c bun -n "__fish_seen_subcommand_from pm; and not __fish_seen_subcommand_from (__fish__get_bun_bins) (__fish__get_bun_scripts) cache;" -a 'bin ls cache hash hash-print hash-string' -f
complete -c bun -n "__fish_seen_subcommand_from pm; and __fish_seen_subcommand_from cache; and not __fish_seen_subcommand_from (__fish__get_bun_bins) (__fish__get_bun_scripts);" -a 'rm' -f
complete -c bun -n "__fish_use_subcommand" -a "create" -f -d "Create a new project from a template"
